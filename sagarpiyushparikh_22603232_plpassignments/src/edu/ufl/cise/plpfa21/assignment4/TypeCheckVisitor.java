...
public class TypeCheckVisitor implements ASTVisitor {
	...
	public Object visitIBinaryExpression(IBinaryExpression n, Object arg) throws Exception {
		...
		if(compatibleAssignmentTypes(e0Type, e1Type)) {
			...
			else if(e0Type.isBoolean() && (op == Kind.AND || op == Kind.OR)) {
				n.setType(PrimitiveType__.booleanType);
			}
			else {
				// not a legal case
...
//			dec.setType(type);
	...
	public Object visitIReturnStatement(IReturnStatement n, Object arg) throws Exception {
		...
		check(compatibleAssignmentTypes(expressionType, resultType), n,
				"Compatiple check in return " );
		return n;
	}
	...
	public Object visitISwitchStatement(ISwitchStatement n, Object arg) throws Exception {
		...
		if(expressionType.isInt() || expressionType.isBoolean() || expressionType.isString()) {
			...
			for (int i = 0; i < branchExpressions.size(); i++) {
				IType actualType = (IType) branchExpressions.get(i).visit(this, arg);
				check(compatibleAssignmentTypes(expressionType, actualType), branchExpressions.get(i),
						"types of actual and formal parameter  are inconsistent");
					check(isConstantExpression(branchExpressions.get(i)),branchExpressions.get(i),
							"types of actual and formal parameter  are inconsistent");
					IBlock block = blocks.get(i);
					block.visit(this, arg);
			}
			IBlock block = n.getDefaultBlock();
			block.visit(this, arg);
